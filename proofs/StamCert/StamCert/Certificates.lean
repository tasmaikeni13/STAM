...
theorem alloc_attained {a b : ℝ} (ha : 0 < a) (hb : 0 < b) :
  ...
  have hbu4 : b * u ^ 4 = 3 * a := by
    rw [hu4]; field_simp
  -- At the optimum the two terms stand in the ratio 1 : 3.
...
theorem debias_unbiased {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω}
  ...
  have hB : ∫ ω, ((e ^ 2 - v) + (2 * e * ξ ω + ξ ω ^ 2)) ∂μ
    ...
    -- `Integrable.add` yields the *pointwise function* sum; re-stating it in lambda form
    -- is what lets `integral_add` unify against the integrand as written.
